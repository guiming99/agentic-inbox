...
function renderCellAttributes(HTMLAttributes: Record<string, unknown>, noBorder = false) {
	if (noBorder) return { ...HTMLAttributes };
...
export const TableRow = Node.create({ name: "tableRow", content: "(tableCell | tableHeader)+", tableRole: "row", addAttributes() { return { height: { default: null, parseHTML: (element: HTMLElement) => element.getAttribute("height") || element.style.height || null, renderHTML: (attributes: { height?: string | null }) => attributes.height ? { height: attributes.height } : {} } }; }, parseHTML() { return [{ tag: "tr" }]; }, renderHTML({ HTMLAttributes }) { return ["tr", HTMLAttributes, 0]; } });

function createTable(schema: Schema, rows: number, cols: number, withHeaderRow: boolean) { const rowNodes = []; for (let row = 0; row < rows; row += 1) { const cellType = row === 0 && withHeaderRow ? schema.nodes.tableHeader : schema.nodes.tableCell; const cells = []; for (let col = 0; col < cols; col += 1) { const cell = cellType.createAndFill({ noBorder: false }); if (cell) cells.push(cell); } rowNodes.push(schema.nodes.tableRow.create(null, cells)); } return schema.nodes.table.create(null, rowNodes); }

export const Table = Node.create({
	name: "table", content: "tableRow+", group: "block", tableRole: "table", isolating: true,
	addAttributes() { return { style: { default: null, parseHTML: (element: HTMLElement) => element.getAttribute("style"), renderHTML: (attributes: { style?: string | null }) => attributes.style ? { style: attributes.style } : {} }, width: { default: null, parseHTML: (element: HTMLElement) => element.getAttribute("width"), renderHTML: (attributes: { width?: string | null }) => attributes.width ? { width: attributes.width } : {} }, columnWidths: { default: null, parseHTML: (element: HTMLElement) => Array.from(element.querySelectorAll(":scope > colgroup > col")).map((col) => { const el = col as HTMLElement; return el.getAttribute("width") || el.style.width || ""; }).filter(Boolean).join(",") || null, renderHTML: (attributes: { columnWidths?: string | null }) => attributes.columnWidths ? { "data-column-widths": attributes.columnWidths } : {} } }; },
	parseHTML() { return [{ tag: "table" }]; },
	renderHTML({ HTMLAttributes, node }) { const columnWidths = typeof node.attrs.columnWidths === "string" ? node.attrs.columnWidths.split(",").filter(Boolean) : []; const colgroup = columnWidths.length ? ["colgroup", ...columnWidths.map((width) => ["col", { style: `width: ${width}` }])] : null; const children = colgroup ? [colgroup, ["tbody", 0]] : ["tbody", 0]; return ["table", mergeAttributes({ style: "border-collapse: collapse; margin: 8px 0;" }, HTMLAttributes), children]; },
	...
	addKeyboardShortcuts() { return { Tab: () => this.editor.commands.goToNextCell(), "Shift-Tab": () => this.editor.commands.goToPreviousCell() }; }, addProseMirrorPlugins() { return [tableEditing()]; },
});
