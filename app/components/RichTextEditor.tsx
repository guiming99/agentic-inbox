...
import { ArrowClockwiseIcon, ArrowCounterClockwiseIcon, ImageIcon, LinkBreakIcon, LinkSimpleIcon, ListBulletsIcon, ListNumbersIcon, MinusIcon, QuotesIcon, TableIcon, TextBIcon, TextItalicIcon, TextStrikethroughIcon, TextUnderlineIcon } from "@phosphor-icons/react";
...
function officeBorderToCss(value: string) {
	const normalized = value.trim();
	if (!normalized) return null;
	const match = normalized.match(/^(?:single\s+)?(?:(\d+(?:\.\d+)?)(?:pt|px)?\s+)?(.+)$/i);
	if (!match) return normalized;
	const width = match[1] ? `${match[1]}pt` : "1px";
	const tail = (match[2] || "solid #808080").trim();
	const style = /dash|dot|double|groove|ridge|inset|outset/i.test(tail) ? tail : "solid";
	const color = tail.match(/#[0-9a-f]{3,8}|rgba?\([^)]*\)|[a-z]+$/i)?.[0] || "#808080";
...
function mergeInlineCss(element: HTMLElement, declarations: string) {
	...
	const addDeclarations = (value: string) => {
		for (const declaration of value.split(";")) {
			...
			if (name === "mso-border-alt") { const border = officeBorderToCss(cssValue); if (border) merged.set("border", border); continue; }
			if (name === "mso-padding-alt") { merged.set("padding", cssValue); continue; }
			if (name === "mso-border-insideh" || name === "mso-border-insidev" || name === "mso-char-indent-count" || name === "mso-line-height-rule") continue;
			if (name.startsWith("mso-")) continue;
...
function resolveOfficeClassStyles(document: Document, table: Element) {
	...
	for (const element of Array.from(table.querySelectorAll("[class]"))) {
		...
		for (const className of classNames) { const declarations = rules.get(className); if (declarations) mergeInlineCss(htmlElement, declarations); }
	}
}

function normalizeOfficeInlineStyles(table: Element) {
	for (const node of Array.from(table.querySelectorAll("td span[style], th span[style], td font, th font"))) {
		const element = node as HTMLElement;
		if (element.tagName.toLowerCase() === "font") {
			const declarations: string[] = [];
			const color = element.getAttribute("color"); if (color) declarations.push(`color:${color}`);
			const face = element.getAttribute("face"); if (face) declarations.push(`font-family:${face}`);
			const size = element.getAttribute("size");
			if (size) { const points = ({ "1": 8, "2": 10, "3": 12, "4": 14, "5": 18, "6": 24, "7": 36 } as Record<string, number>)[size]; if (points) declarations.push(`font-size:${points}pt`); }
			if (declarations.length) mergeInlineCss(element, declarations.join(";"));
		}
		element.setAttribute("data-office-inline-style", "true");
	}
}

function normalizeOfficeCellStyle(element: HTMLElement) {
	const styles = new Map<string, string>();
	const add = (name: string, value: string | null | undefined) => { if (value) styles.set(name, value.trim()); };
	...
	for (const declaration of style.split(";")) {
		...
		if (!value) continue;
		if (name === "mso-border-alt") { const borderValue = officeBorderToCss(value); if (borderValue) add("border", borderValue); continue; }
		if (name === "mso-padding-alt") { add("padding", value); continue; }
		if (name === "mso-border-insideh" || name === "mso-border-insidev" || name === "mso-char-indent-count" || name === "mso-line-height-rule") continue;
		if (name.startsWith("mso-")) continue;
		add(name, value);
	}
	...
	const bg = element.getAttribute("bgcolor"); if (bg) add("background-color", bg);
	const color = element.getAttribute("color"); if (color) add("color", color);
	const font = element.getAttribute("face"); if (font) add("font-family", font);
	const size = element.getAttribute("size");
	if (size && !styles.has("font-size")) { const points = ({ "1": 8, "2": 10, "3": 12, "4": 14, "5": 18, "6": 24, "7": 36 } as Record<string, number>)[size]; if (points) add("font-size", `${points}pt`); }
	const border = element.getAttribute("border"); if (border && !styles.has("border")) add("border", `${border}px solid #808080`);
	if (styles.size) element.setAttribute("style", Array.from(styles.entries()).map(([name, value]) => `${name}: ${value}`).join("; ")); else element.removeAttribute("style");
	for (const attribute of Array.from(element.attributes)) {
		const name = attribute.name.toLowerCase();
		if (name === "class" || (name.startsWith("data-") && name !== "data-office-inline-style") || name.startsWith("mso-") || name === "width" || name === "height" || name === "align" || name === "valign" || name === "bgcolor" || name === "color" || name === "face" || name === "size" || name === "border") element.removeAttribute(attribute.name);
	}
}
...
function normalizePastedTableHtml(html: string) {
	...
	normalizeOfficeInlineStyles(table);
	...
	colgroup?.querySelectorAll("col").forEach((col) => {
		...
		element.removeAttribute("class"); element.removeAttribute("width");
	});
	for (const row of Array.from(table.querySelectorAll("tr"))) {
		for (const cell of Array.from(row.querySelectorAll("td, th"))) {
			...
			if (colspan > 1) element.setAttribute("colspan", String(colspan)); else element.removeAttribute("colspan");
			if (rowspan > 1) element.setAttribute("rowspan", String(rowspan)); else element.removeAttribute("rowspan");
...
export default function RichTextEditor({ value, onChange }: RichTextEditorProps) {
	...
	const editor = useEditor({
		extensions: [StarterKit, SignatureAsset, SignatureRow, ...TableExtensions, Underline, TextAlign.configure({ types: ["heading", "paragraph"] }), LinkExtension.configure({ openOnClick: false }), TiptapImage.configure({ allowBase64: true }), TextStyle, Color, Highlight.configure({ multicolor: true })],
		content: value,
		editorProps: { attributes: { class: "prose prose-sm max-w-none focus:outline-none min-h-[180px] p-3 text-sm [&_blockquote]:border-l-2 [&_blockquote]:border-kumo-line [&_blockquote]:pl-3 [&_blockquote]:text-kumo-subtle [&_blockquote]:bg-kumo-tint [&_blockquote]:py-1 [&_blockquote]:my-2 [&_blockquote]:text-xs [&_blockquote]:rounded-r-sm [&_img]:max-w-full [&_img]:h-auto [&_img]:rounded-sm [&_table]:border-collapse [&_td]:align-top [&_th]:align-top" } },
	...
	const insertImageFile = useCallback(async (file: File) => {
		...
		if (file.size > MAX_INLINE_IMAGE_SIZE) { window.alert(`Image is too large. Please use an image smaller than ${MAX_INLINE_IMAGE_SIZE / (1024 * 1024)}MB.`); return; }
		const reader = new FileReader();
		reader.onload = () => { if (typeof reader.result !== "string" || editor.isDestroyed) return; editor.chain().focus().setImage({ src: reader.result, alt: file.name }).run(); };
	...
	const handlePaste = useCallback((event: ClipboardEvent<HTMLDivElement>) => {
		...
		if (pastedTable && editor) { event.preventDefault(); event.stopPropagation(); editor.chain().focus().insertContent(pastedTable).run(); return; }
		...
		if (!image) return; event.preventDefault(); event.stopPropagation(); void insertImageFile(image);
	...
	return (
		<div className="rounded-lg border border-kumo-line overflow-hidden flex flex-col h-full">
			<div className="flex flex-wrap items-center gap-0.5 bg-kumo-recessed px-2 py-1.5 border-b border-kumo-line shrink-0">
				...
				{tableEditingAvailable && <><Tooltip content="Add row" side="bottom" asChild><Button variant="ghost" shape="square" size="sm" onClick={() => editor.chain().focus().addRowAfter().run()} aria-label="Add row">+R</Button></Tooltip><Tooltip content="Add column" side="bottom" asChild><Button variant="ghost" shape="square" size="sm" onClick={() => editor.chain().focus().addColumnAfter().run()} aria-label="Add column">+C</Button></Tooltip><Tooltip content="Delete row" side="bottom" asChild><Button variant="ghost" shape="square" size="sm" onClick={() => editor.chain().focus().deleteRow().run()} aria-label="Delete row">−R</Button></Tooltip><Tooltip content="Delete column" side="bottom" asChild><Button variant="ghost" shape="square" size="sm" onClick={() => editor.chain().focus().deleteColumn().run()} aria-label="Delete column">−C</Button></Tooltip><Tooltip content="Delete table" side="bottom" asChild><Button variant="ghost" shape="square" size="sm" onClick={() => editor.chain().focus().deleteTable().run()} aria-label="Delete table">×T</Button></Tooltip></>}
			...
			<div className="flex-1 overflow-y-auto" onPasteCapture={handlePaste} onDrop={handleDrop} onDragOver={(event) => { if (Array.from(event.dataTransfer.types).includes("Files")) event.preventDefault(); }}><EditorContent editor={editor} /></div>
		</div>
	);
}
