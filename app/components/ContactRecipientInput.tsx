...
function formatRecipientName(name:string,email:string){const safeName=name.trim().replace(/\\/g,"\\\\").replace(/"/g,'\\"');return safeName?`"${safeName}" <${email}>`:email;}
...
export default function ContactRecipientInput({label,value,onChange,contacts,placeholder,required=false}:{label?:string;value:string;onChange:(value:string)=>void;contacts:RecipientContact[];placeholder:string;required?:boolean}){
 ...
 const add=(raw:string,displayValue?:string)=>{const email=emailFromValue(raw);if(!validEmail(email))return false;if(!recipients.includes(email))onChange([...splitRecipients(value),displayValue||email].join(", "));setInput("");setOpen(false);return true;};
 const commitInput=()=>{const raw=input.trim();if(!raw)return;const parts=splitRecipients(raw);if(parts.length>1){let next=[...splitRecipients(value)];const existing=new Set(recipients);for(const part of parts){const email=emailFromValue(part);if(validEmail(email)&&!existing.has(email)){next.push(part);existing.add(email);}}onChange(next.join(", "));setInput("");setOpen(false);return;}add(raw);};
 const select=(contact:RecipientContact)=>{add(contact.email,formatRecipientName(contact.name,contact.email));inputRef.current?.focus();};
 const remove=(index:number)=>onChange(splitRecipients(value).filter((_,i)=>i!==index).join(", "));
